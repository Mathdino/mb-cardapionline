...
import { Search, Filter, X, Phone, Clock } from 'lucide-react'
              ...
              <tbody className="divide-y">
                {filteredOrders.map(order => (
                  <tr key={order.id} className="hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-foreground">{formatCurrency(order.total)}</td>
            ...
            {filteredOrders.map(order => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2 py-1.5 rounded-full text-sm font-medium ${orderStatusColors[order.status]}`}>
                    {orderStatusLabels[order.status]}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>#{order.id} - {order.items.length} item(s)</span>
            ...
            <div className="relative bg-card rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className={`px-3 py-1.5 rounded-full text-sm font-medium ${orderStatusColors[selectedOrder.status]}`}>
                ...
                {/* Items */}
                <div>
                  <h3 className="font-medium text-foreground mb-2">Itens do Pedido</h3>
                  <div className="space-y-2">
                    {selectedOrder.items.map((item, index) => (
                      <div key={index} className="flex justify-between p-2 bg-secondary/30 rounded-lg">
                        <div>
                          <p className="font-medium text-foreground">{item.quantity}x {item.productName}</p>
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="text-foreground">{paymentMethodLabels[selectedOrder.paymentMethod]}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-bold text-foreground">Total</span>
                    <span className="font-bold text-foreground text-lg">{formatCurrency(selectedOrder.total)}</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-2">
                  {selectedOrder.status === 'pending' && (
                    <>
                      ...
                      <Button variant="outline" className="flex-1 bg-transparent">Cancelar</Button>
                    </>
                  )}
                  {selectedOrder.status === 'confirmed' && (
                    <Button className="w-full">Marcar como Preparando</Button>
                  )}
                  {selectedOrder.status === 'preparing' && (
                    <Button className="w-full">Marcar como Pronto</Button>
                  )}
                  {selectedOrder.status === 'ready' && (
                    <Button className="w-full">Marcar como Entregue</Button>
                  )}
                </div>
              </div>
            </div>
